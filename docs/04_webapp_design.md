...
- 데이터셋 상세 = **샘플 브라우저**: 샘플을 페이지 단위로 넘기며 미니 차트로 개별 검수
  (라벨별 필터, 무작위 샘플링 보기)
- 구현 상태 (M3-A 완료): 프리셋 목록/저장(버전 증가)/보관, 종목 선택 → batch 실행,
  SSE 진행률(종목별 성공/실패 칩), 데이터셋 목록(상태·샘플 수·클래스 분포)까지 동작.
  프리셋 저장 UI는 Lab 파라미터 패널에 있다. 샘플 브라우저는 M3-B.
...
| GET | `/api/datasets/{id}/samples?label=&offset=&limit=` | 샘플 브라우저용 페이지 조회 |
| GET | `/api/datasets/{id}/samples/{i}` | 샘플 1건 상세 (시퀀스 원본/스케일링 값) |
...
| GET/POST | `/api/runs` | 학습 run 목록 / 새 학습 시작 (`{dataset_id, hyperparams}` → job_id) |
...
| **M3** | 프리셋 + 일괄 처리 + 데이터 진단: Supabase preset CRUD, durable job + SSE, private Storage 데이터셋 shard, 샘플 브라우저 + Diagnostics 탭. **M3-A(프리셋·batch·데이터셋 생성) 완료**, M3-B(샘플 브라우저·진단·정리 job) 남음 | 프리셋으로 전체 종목 일괄 전처리 → Supabase 데이터셋 생성/검수 → 품질 리포트 확인 |
| **M4** | 학습 & 평가: `pivot.train` (백로그 A5/A6/B1 베이스라인), Postgres run/epoch/평가 관리 + Storage 체크포인트 + 학습 곡선 SSE, 차트 검증 | 웹에서 학습 시작 → 곡선/지표 확인 → 차트에서 예측 vs 실제 비교 |
