...
- 종목 검색(심볼 마스터) → 수집 대상 추가/제거
  - 생성 폼은 `시장 + 종목명 또는 코드 검색 → 타임프레임 → 수집 기간 → 수집 대상 추가`
    순서이며, 별도 종목코드 입력은 사용하지 않는다.
...
- **타임프레임 선택 수집**: 일봉 / N분봉 / N틱봉 중 선택해 수집
  ...
  - 목록 행은 `시장/거래소 + 종목코드 + 타임프레임 + 요청 시작/종료`로 식별한다.
    같은 종목도 타임프레임이나 요청 기간이 다르면 별도 행으로 모두 표시한다. 추가된 행의 조건은
    불변이며, 행의 수집 버튼은 저장된 조건만 사용한다. 조건 변경은 기존 행 수정이 아니라 새 수집
    대상을 추가하는 방식으로 처리한다.
  - 행별 기간은 공유 parquet에서 잘라 보는 운영 뷰다. 행 삭제 시 동일한 시장·거래소·종목·
    타임프레임을 참조하는 다른 행이 없으면 해당 원천 캐시도 삭제한다.
  - 같은 종목이라도 타임프레임별로 캐시가 분리된다. 일봉은 연 단위, 분봉·틱봉은 일 단위
    parquet 파티션을 사용한다 (`data/raw/kiwoom/{market}/{symbol}/{timeframe}/{partition}/part.parquet`).
  - 국내 market 경로는 `domestic`, 해외는 `overseas/{ND|NY|NA}`를 사용한다.
- 관심종목 테이블: 종목명/코드, **타임프레임별 캐시 상태**(수집 기간, 봉 수, 마지막 갱신), 수집/갱신 버튼
- 수집 요청은 브라우저 FIFO 큐에서 한 건씩 실행한다. 실행 중에도 종목을 추가하거나 다른 종목을
  큐에 넣을 수 있고, 동일한 전체 행 식별자의 중복 요청만 막는다.
- 수집 기간 선택: 시작일/종료일을 직접 지정할 수 있다. 날짜와 시각은 KST 기준이며 분봉·틱봉은
  초 단위까지 입력한다. 미국 종목은 서버가 거래소 시간으로 역변환한다. 미지정 시 기존 캐시 기준
  증분 수집, 지정 시 해당 기간을 조회해 기존 parquet 캐시에 병합한다. 분봉 지정 수집은 MA120
  워밍업을 위해 휴장 시간을 제외한 시작 전 실제 봉 120개도 함께 캐시하되, 차트에는 지정한
  시작·종료 구간만 표시한다.
- 미국 분봉·틱봉은 동부시간 04:00 이상~20:00 미만만 차트·전처리에 사용하고 데이마켓
  (20:00 이상~익일 04:00 미만)은 제외한다.
...
해외 종목은 관심종목 검색·수집·캐시·차트뿐 아니라 전처리 실험실과 데이터셋 생성에도 표시한다.
preview/batch는 종목별 `region/exchange`로 캐시 broker를 결정한다. 원천 캐시 데이터 진단은
현재 국내 종목 식별자 계약을 유지한다. 실시간 탭은 관심종목 목록과 독립적으로 국내·미국
종목마스터 검색에서 구독을 추가한다.

종목 가격을 그리는 모든 차트는 시장별 호가 단위를 일관되게 표시한다. 국내 종목의
가격축은 정수로, 해외 종목의 가격축은 소수점 둘째 자리까지 고정 표기한다. 학습 지표처럼
가격이 아닌 차트에는 이 규칙을 적용하지 않는다.

### 1.2 전처리 실험실 (Lab) — 핵심 화면

```
┌────────────┬──────────────────────────────────────┬───────────────┐
│ 수집 데이터  │  캔들차트 (lightweight-charts)         │ 파라미터 패널   │
│ 리스트      │   - 캔들 + MA 라인(5/20/120)          │  timeframe별 n │
│            │                                      │  tie policy    │
│ (클릭 시    │   - 프랙탈 마커:                       │  피처 선택      │
...
- 왼쪽에는 종목만 중복 제거해 표시하지 않고 Watchlist의 모든 수집 항목을 표시한다. 같은 종목도
  타임프레임이나 요청 시작/종료가 다르면 별도 항목이며, 클릭한 항목의 캐시 범위로 preview한다.
- 학습 피처, 라벨 모드, 최소 스윙 변화율, 샘플 페어링, 동률 극값 처리, 필터와 클리닝은
  타임프레임에 관계없이 프리셋 공통 설정이다. `fractal n`만 `fractal_windows[timeframe_code]`로
  타임프레임별 저장한다. 매핑이 없는 기존 프리셋/타임프레임은 `fractal.n`을 fallback으로 쓴다.
...
- `labeling.min_sequence_length` (기본 `1`): 양 끝 봉을 포함한 입력 시퀀스가 이 봉 수보다
  짧으면 샘플에서 제외한다. 제외해도 도착 마커는 다음 pair의 anchor로 유지하며, preview/batch
  통계의 `dropped_short`에 기록한다. 타임프레임과 무관한 프리셋 공통 설정이다.
...
- 기존 top-level `dropped_ignore`/`dropped_unpaired`는 호환을 위해 유지하고, adjacent 상세는
  ...
  adjacent_edges = samples + dropped_label2 + dropped_nan + dropped_short + dropped_invalid_position
  ```
- 샘플 상세 패널(선택): 해당 시퀀스의 스케일링 후 값 미리보기

### 1.3 일괄 처리 & 데이터셋 (Datasets)

- 프리셋 선택 + 대상(모든 수집 항목 또는 선택) → **일괄 전처리 실행**. 대상은
  `종목+시장/거래소+타임프레임+요청 시작/종료`의 복합 식별자를 사용한다.
- 한 데이터셋에 일봉·분봉·틱봉을 함께 포함할 수 있다. 단일 타임프레임이면 기존 코드를,
  둘 이상이면 데이터셋 메타 `timeframe="mixed"`를 기록한다.
- 진행 상황: SSE로 수집 항목별 진행률/성공/실패/샘플 수 스트리밍
- 완료 시 데이터셋 카드 생성: 이름, 생성일, 사용 프리셋(파라미터 스냅샷), 샘플 수, 클래스 분포
- ready 데이터셋의 `확장`은 기존 프리셋 스냅샷과 대상을 고정하고 새 수집 대상을 합쳐
  별도 데이터셋을 생성한다. 기존 데이터셋/shard는 수정하지 않으며, 전체 샘플의 클래스별
  60/20/20 split을 다시 계산한다. 새 스냅샷은 `extended_from_dataset_id`를 기록한다.
...
- 데이터셋 진단:
  ...
  - train/val/test split이 저장된 클래스별 샘플 60/20/20 규칙과 일치하는지 검사
...
- 학습 입력은 `ready` 데이터셋 shard의 저장된 샘플 split만 사용한다. 세 split이 비었거나 shard
...
- **가변 길이**: 0 패딩 + `mask`/`length`를 함께 전달해 패딩이 손실·지표에 영향을
  주지 않게 한다 (백로그 A3).
- **split**: 전체 샘플을 클래스별로 결정적 셔플한 뒤 각 클래스 안에서 **60/20/20**으로 나눈다.
  결과는 shard 행의 `split`에 저장하고 학습 코드는 재분할하지 않는다. 기존 종목 단위
  `seeded_shuffle_v1/v2` 데이터셋은 저장된 배정을 그대로 읽는다.
...
- **split 재현성**: shard의 샘플 split이 클래스별 60/20/20 seed 재계산과 다르면 로더/진단이 거부
...
증권사 연결은 **broker-modules Kiwoom WebSocket 국내 `주식체결(0B)` / 미국
`해외주식체결(FE)`**로 고정한다.

활성 탭은 브라우저 로컬 상태에 저장해 페이지 새로고침 후에도 같은 작업 화면을 복원한다.
...
- 데이터 경로: **Kiwoom WebSocket ↔ FastAPI(중계·집계·추론) ↔ 브라우저 WebSocket**
  ...
  - FastAPI lifespan에서 Kiwoom client 하나와 KRX/US 시장별 session을 유지하고 국내
    `subscribe_trades`, 미국 `subscribe_us_trades`를 동적으로 관리
  ...
  - 미국 봉 경계는 America/New_York로 집계하고 REST/WS 분봉 시간은 KST로 변환해 표시
...
- 활성 모델 timeframe에서는 `시작 앵커 지정` 후 차트의 과거 마감 봉을 클릭해 종목별 수동
  입력 시작점을 지정할 수 있다. 수동 시작점은 자동 프랙탈·예측 anchor보다 우선하고 보라색
  마커로 표시하며, `자동 앵커`로 기존 pairing 규칙을 복원한다.
...
// Supabase public.training_presets.preset
{
  ...
  "fractal": {
    "n": 20,                                      // 기존 프리셋/미지정 timeframe fallback
    "tie_policy": "plateau_last"                 // 동률 plateau의 마지막 봉만 라벨
  },
  "fractal_windows": {                            // 유일한 timeframe별 전처리 설정
    "day": 20,
    "min1": 15,
    "tick30": 9
  },
  ...
  "labeling": {                                    // 백로그 B2 실험을 위해 모드화
    ...
    "min_sequence_length": 1                       // 이보다 짧은 입력 시퀀스는 제외
  },
  "filters": {
...
| GET/POST | `/api/watchlist` | 타임프레임·요청 기간을 포함한 수집 데이터 항목 목록/추가 |
| DELETE | `/api/watchlist/{symbol}?timeframe=&start=&end=&region=&exchange=` | 정확히 일치하는 항목 삭제. 같은 캐시 참조가 없으면 로컬 parquet도 삭제 |
...
| POST | `/api/preprocess/batch` | 신규 `{preset_id, dataset_name, targets[]}` 또는 확장 `{base_dataset_id, dataset_name, targets[]}` → durable job_id |
...
| DELETE | `/api/live/model` | 활성 모델 해제 후 전체 실시간 상태 반환 |
| PUT/DELETE | `/api/live/anchors/{symbol}` | 종목별 수동 추론 시작 앵커 설정/자동 앵커 복귀 |
| GET/POST | `/api/live/subscriptions` | Kiwoom 국내 `0B`/미국 `FE` 구독 종목 조회/추가 |
...
- 시간값: 일봉은 `'yyyy-mm-dd'` 문자열, 분봉/틱봉은 KST 벽시계를 UTC 필드로 복원하는
  ...
  미국 일봉의 문자열은 정규장 종료 시각이 속하는 KST 날짜이며, 미국 분봉·틱봉과 전처리/학습
  차트도 America/New_York 원본을 KST로 변환한 동일 시간축을 사용한다.
...
| **M1** | 수집: broker-modules 연동(Kiwoom 일/분/틱), 기간 선택 수집, watchlist, parquet 캐시, 차트+보조지표(MA/거래량) 표시/제거, MA 기간 추가/삭제 | 종목·타임프레임·기간으로 수집 대상 추가 → 수집 → 실데이터 차트와 선택 보조지표 확인 |
...
| **M5** | 실시간 추론: Kiwoom WebSocket 국내 `0B`/미국 `FE` 중계, 거래소 시간대별 봉 집계 + KST 차트 + 실시간 추론, Live 화면. **core+UI·원격 migration·장외 회귀 검증 완료, 모델 활성화·국내/미국 장중 검증 대기** (2026-07-14) | 장중에 구독 종목의 현재 봉과 실험적 후보 점수가 KST 차트에 표시되고 재접속 후 누락 없이 복구 |
