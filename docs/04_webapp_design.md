...
- **타임프레임 선택 수집**: 일봉 / N분봉 / N틱봉 중 선택해 수집
  ...
  - 목록 행은 `시장/거래소 + 종목코드 + 타임프레임 + 요청 시작/종료`로 식별한다.
    같은 종목도 타임프레임이나 요청 기간이 다르면 별도 행으로 모두 표시하며, 상단 타임프레임과
    기간 컨트롤은 기존 행을 필터링하지 않고 새 데이터 항목을 추가할 때만 사용한다.
  - 행별 기간은 공유 parquet에서 잘라 보는 운영 뷰다. 행 삭제는 목록 메타데이터만 제거하고
    다른 행과 공유할 수 있는 원천 캐시는 삭제하지 않는다.
...
| DELETE | `/api/watchlist/{symbol}?timeframe=&start=&end=&region=&exchange=` | 정확히 일치하는 수집 데이터 항목 삭제 |
