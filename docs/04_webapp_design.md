...
- `labeling.ignore_swing_pct` (선택): 스윙 시작(직전 반대 프랙탈)과 끝(현재 프랙탈)의
  가격 변화율이 이 값(%) 미만이면 잔진동으로 보고 **무시(2)로 라벨**한다. 역배열 무시
  규칙과 독립적으로 조합된다. cls2_drop 모드에서는 해당 지점이 제외되며, 제외된 지점은
  이후 스윙 진폭 평가의 anchor가 되지 않는다. 필드가 없는 기존 프리셋은 미적용(null)으로
  읽어 동작이 변하지 않는다.
...
- 데이터셋 상세 = **샘플 브라우저**: 샘플을 페이지 단위로 넘기며 미니 차트로 개별 검수
  (라벨별 필터, 무작위 샘플링 보기)
- 구현 상태 (M3 완료): 프리셋 목록/저장(버전 증가)/보관, 종목 선택 → batch 실행,
...
- 구현 상태 (M3 완료): 검사 로직은 `pivot/diagnostics/quality.py`(순수 함수),
  ...
  진단은 동기 실행이다(로컬 단일 사용자 규모) — 오래 걸리면 job+SSE로 전환한다.
...
// Supabase public.training_presets.preset
{
  ...
  "labeling": {                                    // 백로그 B2 실험을 위해 모드화
    ...
    "ignore_swing_pct": null                       // 스윙 시작~끝 변화율(%)이 미만이면 무시(2), null이면 미적용
  },
  "filters": {
...
| **M5** | 실시간 추론: Kiwoom WebSocket `0B` 중계, timeframe별 봉 집계 + 실시간 추론, Live 화면 | 장중에 구독 종목의 현재 봉과 실험적 후보 점수가 차트에 표시되고 재접속 후 누락 없이 복구 |
