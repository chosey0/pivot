...
| **M3** | 프리셋 + 일괄 처리 + 데이터 진단: Supabase preset CRUD, durable job + SSE, private Storage 데이터셋 shard, 샘플 브라우저 + Diagnostics 탭 | 프리셋으로 전체 종목 일괄 전처리 → Supabase 데이터셋 생성/검수 → 품질 리포트 확인 |
