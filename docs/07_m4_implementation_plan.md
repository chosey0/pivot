...
2. 학습은 FastAPI 이벤트 루프와 분리된 프로세스에서 실행된다.
   FastAPI 정상 종료 시 실행 중인 학습 프로세스를 종료·대기해 고아 프로세스로 남기지 않는다.
...
- 전체 샘플을 클래스별로 결정적 셔플해 `train`/`validation`/`test = 60/20/20`으로 나누고
  shard 행에 저장된 split을 그대로 사용한다. 기존 종목 단위 v1/v2 데이터셋은 재현만 한다.
