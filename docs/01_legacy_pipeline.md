...
**핵심 성질**: 프랙탈은 중심 봉 앞뒤의 봉을 봐야 확정되는 **후행 지표**다.
라벨은 미래 정보로 확정하되, 모델 입력은 해당 시점까지의 과거 데이터만 사용하므로
"이 봉이 나중에 프랙탈 고점/저점으로 확정될 것인가"를 예측하는 문제가 된다.

> 창 정렬 정밀화 (pivot 재구현에서 실측 고정): pandas `rolling(n, center=True)`는
> 짝수 n에서 남는 한 봉을 **과거 쪽**에 붙인다 — 창 = 과거 `n//2` + 중심 + 미래 `(n-1)//2`.
> 따라서 확정 lag는 정확히 `(n-1)//2`봉이다 (n=20 → 9봉, 홀수 n은 `n//2`와 동일).
> 재구현(`pivot/labeling/fractal.py`)은 이 정렬을 그대로 따르고 tests로 고정했다.
> 시리즈 마지막 `(n-1)//2`봉은 미확정 구간으로 절대 라벨하지 않는다.

> 동률 극값 정규화 (pivot 품질 보완): legacy는 rolling 최댓값/최솟값과 같은 봉을
> 모두 프랙탈로 마킹해 횡보 plateau에서 하루씩 어긋난 유사 샘플을 만들었다. 신규 프리셋의
> 기본 `tie_policy=plateau_last`는 같은 종류·같은 가격의 연속 후보를 하나의 event로 묶고
> 마지막 봉만 라벨한다. 과거 저장 프리셋에서 이 필드가 누락된 경우에는 재현성을 위해
> legacy `all`로 해석한다.
...
### 4.1 Pivot 재구현의 확정 샘플 페어링 계약 (구현 완료)
