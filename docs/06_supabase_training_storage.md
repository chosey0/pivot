...
- 데이터셋 확장은 기존 행이나 shard에 append하지 않는다. ready 데이터셋의 불변 프리셋
  스냅샷/대상과 새 대상을 합쳐 전체를 다시 처리한 새 데이터셋을 만들고,
  `preset_snapshot.extended_from_dataset_id`로 계보를 기록한다. 따라서 보관된 프리셋으로 만든
  데이터셋도 당시 스냅샷 그대로 확장할 수 있다.
...
2. 전 수집 항목을 최대 2개씩 병렬 전처리해 클래스별 `(source_key, sample_index)`를 모으고 60/20/20 split을 확정한다. 이 결과는 같은 batch 작업 안에서만 재사용한다.
3. 수집 항목별 전처리 결과를 확정된 `split`과 함께 parquet shard로 직렬화하고 SHA-256과 행 수를 계산한다
