...
2. 전 종목을 1차 전처리해 클래스별 샘플 키를 모으고 60/20/20 split을 확정한다.
3. 종목별 샘플을 다시 전처리해 확정된 `split`과 함께 parquet shard로 직렬화하고 SHA-256과 행 수를 계산한다
...
- **학습 run 삭제** (`DELETE /api/runs/{id}`): queued/running run과 `live_deployments` 이력에서
  참조 중인 run은 거부한다. terminal run은 artifact의 bucket/path 목록을 `run_delete` job에
  확정하고 Storage 객체를 먼저 삭제한 뒤 run 메타데이터를 삭제한다. epoch/evaluation/artifact는
  FK cascade로 함께 제거되고 원래 training job은 감사 이력으로 남는다. `run_delete` kind는
  `20260713133933_run_delete_job_kind.sql`이 추가하며 원격 적용을 완료했다.
