...
(구 max_len 고정 길이 윈도우는 폐기했다. 신규 기본 `adjacent_markers_v1`은 필터 후 남은
마커를 시간순 인접 pair로 연결하며, 같은 종류 pair는 무시(2), 다른 종류 pair는 도착 마커
기준 저점(0)/고점(1)으로 라벨한다. 기존 저장 프리셋은 `latest_opposite_v1`로 재현한다.
docs/01 §4.1, docs/04 §1.2 참고)
