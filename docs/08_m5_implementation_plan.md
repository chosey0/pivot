...
2. 국내 종목마스터 검색에서 선택한 종목을 실시간 구독하면 서버가 Kiwoom `주식체결(0B)`을 수신한다.
...
- 활성 모델 timeframe의 Live 이력은 checkpoint의 프리셋 스냅샷으로 계산한
  `fractal_markers`를 함께 반환한다. 실제 프랙탈은 `H/L` 화살표, 모델 판정은 `예측 H/L`
  원형 마커로 구분하며 이후 확정된 실제 포인트는 prediction 후보 anchor를 재사용한다.
...
| POST | `/api/live/subscriptions` | `{symbol}`. 구독 후 갱신된 구독 목록 반환 |
...
공개 run·dataset·artifact id, model/timeframe/features/pairing만 포함한다. 구독 행은 전송 상태
...
분봉·틱봉은 unix 초를 사용한다. prediction의 `scores`는 고정 순서
...
## 9. 현재 구현 상태 (2026-07-13)

M5-A~E의 core와 Live UI 통합은 완료했다. core는 실시간 봉 집계, 학습 snapshot 기반 추론,
Kiwoom 단일 session, REST cache reconcile, 단일 활성 deployment, HTTP/WS fan-out을 제공한다.
...
실시간 구독 추가는 관심종목 목록에 의존하지 않고 국내 종목마스터 퍼지 검색을 직접 사용하며,
자동완성에서 선택된 6자리 종목코드만 구독 API로 전달한다.
