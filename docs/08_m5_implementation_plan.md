...
- 활성 모델이 분봉이고 해당 종목의 local raw 분봉 캐시가 없으면, 최초 reconcile은 해당 시장
  시간 기준 최근 1일만 조회한다. 이후 reconcile은 마지막 캐시 봉 이후만 증분 조회한다. 이는
  실시간 워밍업이 전체 과거 분봉 적재에 막히지 않도록 하는 초기화 한도이며, Live 화면 과거
  차트의 별도 Kiwoom REST 페이지 조회 정책에는 영향을 주지 않는다.
