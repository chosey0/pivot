...
- 수집 기간은 선택적으로 지정할 수 있다. 브라우저와 `/api/ingest`의 `start`/`end`는 모두
  **대한민국 시간(KST)** 기준이다. 일봉은 날짜(`YYYY-MM-DD`)를, 분봉·틱봉은 초 단위 시각
  (`YYYY-MM-DDTHH:mm:ss`)을 받는다. 미국 분봉·틱봉은 서버가 America/New_York로 역변환한 뒤
  SDK 조회와 정확한 시작·종료 필터에 사용한다. 미국 일봉의 KST 날짜는 해당 날짜에 정규장
  종료가 속하는 미국 거래일로 역변환한다.
  분봉은 시작 시점의 MA120도 계산할 수 있도록 이전 거래일까지 조회 범위를 넓힌 뒤 지정 시작
  전의 실제 봉 120개만 함께 캐시한다. 휴장 시간은 워밍업 봉으로 세지 않으며 종료 시각과
  틱봉/일봉 범위는 확장하지 않는다. 차트 API는 이 선행 봉으로 MA를 계산한 뒤 사용자가 지정한
  수집 시작·종료 구간만 응답하므로 워밍업 봉은 렌더링하지 않는다.
...
- 미국 분봉 원본은 연장시간을 `24` 이상 시각(예: `20260710274300`)으로 표현할 수 있다.
  ...
  미국 데이마켓(동부시간 20:00 이상~익일 04:00 미만)은 학습·차트 대상에서 제외한다. 기존
  분봉·틱봉 캐시는 차트·전처리에서 즉시 제외하고 다음 수집 시 같은 규칙으로 파일도 정리한다.
  프리마켓 04:00부터 애프터마켓 20:00 직전까지는 유지한다.
  parquet 원본과 봉 경계는 America/New_York를 유지하고, 일반 차트·전처리 미리보기·학습 예측
  차트·Live REST/WS 응답은 KST로 변환한다. 분봉·틱봉은 KST 벽시계 unix 초를 사용하고,
  일봉은 미국 정규장 종료 시각이 속하는 KST 날짜를 사용한다.
